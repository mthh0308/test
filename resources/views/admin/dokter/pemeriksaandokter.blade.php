...
@section('container')
    <div class="card">
        <div class="card-body mt-3">
            <h3>Halaman Pemeriksaan Dokter</h3>
            <hr>
            <h6>Data Pemeriksaan Dokter</h6>
        </div>
    </div>
    <div class="container">
        ...
        @endif
        <a href="/pemeriksaandokter" class="btn btn-info btn-sm"><i class="bi bi-arrow-repeat"></i> Refresh</a>
    </div>
    <!-- Filter Tanggal -->
    ...
    <div class="search-bar row g-3 d-flex flex-row-reverse mb-3">
        <div class="col-auto">
            <form class="search-form" action="/pemeriksaandokter" method="get">
                    ...
                    <tbody>
                        ...
                        @foreach ($pasien as $index => $w)
                            <tr>
                                ...
                                <td>{{ $w->no_hp }}</td>
                                <td>
                                    <span class="badge {{ $w->status_pemeriksaan_dokter == 'Sudah Diperiksa' ? 'bg-success' : 'bg-danger' }}">
                                        {{ $w->status_pemeriksaan_dokter }}
                                    </span>
                                </td>                                                                
                                <td>
                                    <a href="/inputpemeriksaandokter/{{ $w->id }}" type="button"
                                        class="btn btn-warning">Resume Medis</a>
                                    <a href="/detailpemeriksaandokter/{{ $w->id }}" type="button"
