...
@section('container')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ url('aksi_edit_resep/' . $dr->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-md-12 mb-1">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="datetime-local" name="tglresep" class="form-control" id="tglresep"
                            value="{{ \Carbon\Carbon::parse($dr->tglresep)->format('Y-m-d\TH:i') }}">
                    </div>
                    <div class="col-md-2">
                        ...
                        <select name="id_obat" class="form-select" id="id_obat" required>
                            ...
                            @foreach($obat as $o)
                                <option value="{{ $o->id }}" {{ $o->id == $dr->id_obat ? 'selected' : '' }}>
                                    {{ $o->nama_obat }}
                                </option>
                            @endforeach
                        </select>                        
